...
unnormalize_rgb = transforms.Normalize(mean=[-0.485 / 0.229, -0.456 / 0.224, -0.406 / 0.225],
                                       std=[1 / 0.229, 1 / 0.224, 1 / 0.225])
# unnormalize_rgb = transforms.Compose([ transforms.Normalize(mean = [ 0., 0., 0. ], std = [ 1/0.229, 1/0.224, 1/0.225 ]),
...
def jpeg_compress(x, quality_factor):
    ...
    for ii, img in enumerate(x):
        ...
        # plt.imshow(pil_img)
        # plt.show()
        img_aug[ii] = transform_img(aug_functional.encoding_quality(pil_img, quality=quality_factor))
...
def gaussian_blur(x, sigma=1, kernel_size=21):
...
def gaussian_noise(x, mean=0.0, std=0.1):
    ...
    transform = transforms.Compose([
        ...
        GaussianNoise(mean=mean, std=std),  # 添加高斯噪声
        # transforms.ToPILImage()  # 转回 PIL 图像以便显示
    ])
    x = transform(x)

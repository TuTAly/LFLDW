...
def sample_from_tensors(filename='stage1_35/ori_latent', sample_num=18, seed=None):
    ...
    # 获取文件夹中所有的张量文件名
    # Gets all tensor file names in the folder
    tensor_files = [f for f in os.listdir(tensor_folder_path) if f.endswith('.pt')]

    # 定义要随机选取的张量数量
    # Define the number of tensors to be picked at random
    x = sample_num  # 替换为您需要的数量

    # 确保 x 不超过文件总数
    # Make sure x does not exceed the total number of files
    x = min(x, len(tensor_files))

    # 随机选取 x 个张量文件
    # X number of tensor files are randomly selected
    selected_files = random.sample(tensor_files, x)

    # 初始化一个列表来存储加载的张量
    # Initialize a list to store loaded tensors
    tensors = []

    # 批量加载选取的张量文件
    # Load selected tensor files in batches
    ...
    # 将选取的张量合并为一个大的四维张量
    # 假设所有张量具有相同的形状
    # Merge selected tensors into one large four-dimensional tensor
    # Assuming that all tensors have the same shape
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params, pipe, is_mask=True):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        attacks = {
            ...
            'gaussian_noise': lambda x: utils_img.gaussian_noise(x, std=0.3),
        }
        ...
        for name, attack in attacks.items():
            ...
            imgs_aug_img = tensor_to_image(imgs_aug[0])
            plt.imshow(imgs_aug_img,cmap='gray')
            plt.show()

            file_path = f"Val_att/attacked_image_epoch_{name}.png"
            imgs_aug_img.save(file_path)

...
def get_parser():
    ...
    aa("--channel_num", type=int, default=3, help="The channel of watermark embedder")
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params, pipe, data_aug,
    ...
    for it, (_, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        temp1 = torch.unsqueeze(latents[:, params.channel_num], dim=1).real
        temp2 = torch.unsqueeze(latents[:, params.channel_num], dim=1).imag
        ...
        latents[:, params.channel_num] = complex
        ...
        temp1 = torch.unsqueeze(inverted_latents[:, params.channel_num], dim=1).real
        temp2 = torch.unsqueeze(inverted_latents[:, params.channel_num], dim=1).imag
        temp = torch.cat((temp1, temp2), dim=1)

...
def get_parser():
    ...
    aa("--p_rot", type=float, default=0, help="Probability of the rotation augmentation. (Default: 0.5)")
    aa("--p_color_jitter", type=float, default=1,
...
def train_one_epoch(encoder_decoder: models.EncoderDecoder, loader, optimizer, scheduler, epoch, params, pipe, data_aug,
    ...
    for it, (_, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        current_prompt = []
        prompt = 'realistic painting of a tardigrade kaiju, with 6 legs in a desert storm, by james gurney, slime, big globule eye, godzilla, vintage, concept art, oil painting, tonalism, crispy'

        # prompt = dataset[it][prompt_key]

        for i in range(params.batch_size):
            current_prompt.append(prompt)

        # change the param stop_step to determine the injecting watermark timestep
        outputs_latents_no_w = sample_from_tensors(sample_num=params.batch_size, seed=it)
        ...
        file_path = f"G/ori_image_epoch_{epoch}iteration_{it}.png"
        ...
        file_path = f"G_W/w_image_epoch_{epoch}iteration_{it}.png"
...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params, pipe, is_mask=True):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        current_prompt = []
        # prompt = 'a very beautiful anime cute girl, a very beautiful anime'
        prompt = 'realistic painting of a tardigrade kaiju, with 6 legs in a desert storm, by james gurney, slime, big globule eye, godzilla, vintage, concept art, oil painting, tonalism, crispy'
        for i in range(params.batch_size_eval):
            current_prompt.append(prompt)

        # change the param stop_step to determine the injecting watermark timestep
        outputs_latents_no_w = sample_from_tensors(filename='stage1_35/test', sample_num=params.batch_size_eval,
                                                   seed=it)
        ...
        # show and save original image
        orig_image_no_w = outputs_no_w.images[0]
        file_path = f"Val/ori_image_epoch_{epoch}iteration_{it}.png"
        orig_image_no_w.save(file_path)
        # plt.title('origianl image')
        # plt.imshow(orig_image_no_w)
        # plt.show()
        ...
        # show and save watermarked image
        orig_image_w = outputs_w.images[0]
        file_path = f"Val_w/w_image_epoch_{epoch}iteration_{it}.png"
        orig_image_w.save(file_path)
        ...
        attacks = {
            ...
            'gaussian_noise': lambda x: utils_img.gaussian_noise(x),
        }
        ...
        for name, attack in attacks.items():
            ...
            attack_word_accs = (attack_bit_accs == 1)  # b

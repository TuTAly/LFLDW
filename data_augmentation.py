...
class HiddenAug(nn.Module):
    ...
    def __init__(self, img_size, p_crop=0.3, p_blur=0.3, p_jpeg=0.3, p_rot=0.3, p_color_jitter=0.3, p_res=0.3,p_noise=0.3):
        super().__init__()
        augmentations = []
        ...
        if p_noise>0:
            noise1 = K.RandomGaussianNoise(mean=0,std=0.1,p=1)
            noise2 = K.RandomGaussianNoise(mean=0,std=0.3,p=1)
            augmentations += [noise1, noise2]

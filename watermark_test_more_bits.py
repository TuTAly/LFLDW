...
def eval_one_epoch(encoder_decoder: models.EncoderDecoder, loader, epoch, params, pipe, is_mask=True):
    ...
    for it, (imgs, _) in enumerate(metric_logger.log_every(loader, 10, header)):
        ...
        msgs_ori = torch.rand((outputs_latents_no_w.shape[0], params.num_bits * 2)) > 0.5  # b k
        msgs = 2 * msgs_ori.type(torch.float).to(device) - 1  # b k

        msgs_0, msgs_1 = torch.split(msgs, 8, dim=1)
        ...
        complex = add_noise(latents, msgs_0, r1=params.r1, w_weight=params.w_weight, channel=0)
        complex = add_noise(complex, msgs_1, r1=params.r1, w_weight=params.w_weight, channel=1)
        ...
        temp0 = torch.unsqueeze(inverted_latents[:, 0], dim=1).real
        temp1 = torch.unsqueeze(inverted_latents[:, 1], dim=1).real
        temp2 = torch.unsqueeze(inverted_latents[:, 1], dim=1).imag
        temp = torch.cat((temp1, temp2), dim=1)
